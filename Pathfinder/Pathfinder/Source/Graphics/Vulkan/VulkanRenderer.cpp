...
const uint64_t g_MaxEntities = 500;
...
void VulkanRenderer::RegisterEntity(const Entity* const _entity)
{
...
void VulkanRenderer::RecordRenderCommands(const uint8_t _imgIndex)
{
	...
	{
		m_VertexBufferManager->Bind(static_cast<uint32_t>(m_Entities[i]->GetShape()), cmdBuffer);
		...
		// Push constants
		glm::mat4 modelMatrix = m_Entities[i]->m_Transform.GetModel();
		vkCmdPushConstants(cmdBuffer, m_VkGraphicsPipeline->GetLayout(), VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(ModelMatrix), &modelMatrix);

		m_Entities[i]->Draw(cmdBuffer, m_VertexBufferManager->GetCurrentIndicesCount());
	}
